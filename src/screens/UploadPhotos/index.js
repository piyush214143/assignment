...
import { fetchData, uploadImages } from '../../slice/ApiCalling';
...
const UploadPhotos = (props) => {
  ...
  const uploadBeforeImages = () => {
    console.log("image recieved hereeee================>>>>>>>>>>>>>>", pickerImage)
    const formData = new FormData()
    formData.append("user_id", usedID)
    // formData.append("uploadedImages" , pickerImage?.filePath?.assets[0] )
    // formData.append("uploadedImages", pickerImage)
    formData.append('myImage', {
      uri: pickerImage.uri,
      type: pickerImage.type,
      name: pickerImage.fileName,
    });
    formData.append("photo_type", "before")
    formData.append("created_date", "08-09-2023")
    // console.log("lllllllllllllllll", Object.fromEntries(formData));
    dispatch(uploadImages()).then(result => {
  ...
  console.log("picker image we got", pickerImage)
  return (
